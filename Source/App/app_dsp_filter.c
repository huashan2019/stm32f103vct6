...
SCH_BOOL Dsp_GEN_Filter(SCH_U8 Channel,Filter_HPLP HPLP,Filters_T *P_Filter)
{
	...
	for(index=0;index<4;index++)
	{
		...
		if(Channel < 8)/*0~7 dsp1;8~15 dsp2*/
		SIGMA_SAFELOAD_WRITE_REGISTER(SCH_DSP1,DEVICE_ADDR_IC_1, GEN_Filter_addr[Channel][HPLP][index],   5, buff);
		else
		SIGMA_SAFELOAD_WRITE_REGISTER(SCH_DSP2,DEVICE_ADDR_IC_1, GEN_Filter_addr[Channel][HPLP][index],   5, buff);
	}
	if((PHASE_Flag)&&(App_Dsp.Dsp_Data.Mute[Channel+1]==DSP_UNMUTE))
	{
		SysWaitMs(30);
	...
	App_Printf("\r\n Filter data: Channel=%d,HPLP=%x,filter=%x",Channel,HPLP,*P_Filter);
	return TRUE;
}
