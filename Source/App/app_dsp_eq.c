...
SCH_BOOL Dsp_EQ_Set(SCH_U8 Channel,EQ_NUM_T EQ_NUM,EQ_T *P_EQ)
{
	...
	Addr = EQ_Set_addr[(Channel%8)*2+EQ_NUM/16]+EQ_NUM%16*5;
	...
	if(Channel < 8)
		SIGMA_SAFELOAD_WRITE_REGISTER(SCH_DSP1,DEVICE_ADDR_IC_1, Addr,   5, buff);
	else
		SIGMA_SAFELOAD_WRITE_REGISTER(SCH_DSP2,DEVICE_ADDR_IC_1, Addr,   5, buff);
